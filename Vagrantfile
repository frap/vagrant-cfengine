...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.box = "centos-6.5-x86_64-cfengine_enterprise-vagrant-#{BASEBOX_BUILD}"
  ...
  # vm communication
  first_three_network_octets = "172.16.8"

  # We reserve the hub ip for .2
  #cfengine_hub_ip = "#{first_three_network_octets}.2"
  #aws_hub_ip      = "54.153.205.217"
  hub_ip           = "192.168.66.225"
 ...
 #   hub.vm.network :forwarded_port, guest: 80, host: hub_80_fwd
  ...
  #  end
   ...
   #     v.name = "CFEngine Ent #{PACKAGE_RELEASE}-#{PACKAGE_BUILD} hub"
   # end
   ...
   # hub.vm.provision :shell, :path => "scripts/install_cfengine_enterprise.sh", :args => "hub #{PACKAGE_RELEASE} #{PACKAGE_BUILD}"
  ...
  (1..hosts).each do |host_number|
    ...
    host_name = "agent" + host_number.to_s.rjust(3, '0')
    ...
    config.vm.define host_name.to_sym do |host|
      ...
      host.vm.provision :shell, :path => "scripts/install_cfengine_enterprise.sh", :args => "agent #{PACKAGE_RELEASE} #{PACKAGE_BUILD}"
      host.vm.provision :shell, :path => "scripts/bootstrap_cfengine.sh", :args => "#{hub_ip}"
      ...
      host.vm.provider "virtualbox" do |v|
        ...
        #v.gui=true
        v.name = "CFEngine #{PACKAGE_RELEASE}-#{PACKAGE_BUILD} #{host_name}"
      end
